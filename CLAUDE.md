...
- branches homologados parcialmente (2026-09-19): `sb_list_branches`/`sb_create_branch`/`sb_delete_branch` confirmados reais; `sb_merge_branch`/`sb_reset_branch`/`sb_rebase_branch` só tiveram o gating de confirmação validado, nunca executados contra a API real (merge afeta produção, reset descarta dados) — ver `04_SDD` seção 9;
- **incidente de segurança real (contido) registrado**: `GET /branches/{id}` devolve `db_pass`/`jwt_secret` em texto claro (não são metadados de branch) — descoberto e contido durante a homologação, nenhuma tool exposta chama esse endpoint hoje. Ver `01_RAG` RAG-007A e `04_SDD` T-008 antes de adicionar qualquer tool `sb_get_branch` no futuro;
