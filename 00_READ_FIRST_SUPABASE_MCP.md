...
**Homologação de branches (2026-09-19), com um incidente de segurança real registrado no processo**:
- `sb_list_branches` → `GET /projects/{ref}/branches`: **correto**, confirmado (200, retornou a branch `main` real).
- `sb_create_branch` → `POST /projects/{ref}/branches` com `{"branch_name"}`: **correto**, confirmado (201, criou uma branch de teste real, provisionando um sub-projeto efêmero com seu próprio `project_ref`).
- `sb_delete_branch` → `DELETE /branches/{branch_id}` (o `id` da branch, não o `project_ref` do sub-projeto): **correto**, confirmado (200, branch de teste removida, limpeza completa).
- `sb_merge_branch`/`sb_reset_branch`/`sb_rebase_branch`: **não executados contra a API real, por decisão deliberada** — `merge` aplica mudanças na produção de verdade, `reset` descarta dados da branch; nenhum dos dois é seguro de "só testar" contra um projeto real, mesmo de baixa criticidade. Validado apenas o **bloqueio de confirmação** (sem `confirmation`, todas as três rejeitadas antes de qualquer chamada HTTP acontecer — nenhuma rede foi tocada). Os paths desses três continuam **não confirmados** contra a API real — ver `04_SDD` seção 9.

**Incidente de segurança real (contido) durante a investigação (2026-09-19)**: ao testar `GET /branches/{id}` (para descobrir se esse formato de endpoint existia, antes de decidir usá-lo em alguma tool), a Management API devolveu — sem eu esperar — as credenciais completas do banco daquela branch (`db_pass` e `jwt_secret` em texto claro), que apareceram na saída de um script de diagnóstico e ficaram visíveis na conversa. A branch em questão já tinha sido apagada na sequência do mesmo teste, então o banco por trás dessas credenciais não existe mais — não há nada para rotacionar. Não veio de nenhuma tool exposta deste MCP (não existe `sb_get_branch` no catálogo) e não passou pelo sistema de auditoria (`audit.py` já redige `db_pass`/`jwt_secret` por nome de campo — o vazamento foi só na saída bruta do script de investigação, que não passa por `write_audit()`). Lição registrada permanentemente: `01_RAG` RAG-007A, `04_SDD` T-008.
...
2. ~~Homologação de branches de desenvolvimento~~ — feita em 2026-09-19 (parcial, por decisão consciente), ver evidência abaixo.
